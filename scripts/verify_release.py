...
PENDING_DISAGREEMENTS = (
    PendingDisagreement(
        what="东财历史资金流 · 个别已收盘日期有两个版本，接口逐次返回其中之一",
        values="SH600362 2026-07-01 主力净流入在 -4342.94万(-1.93%) 与 -3982.64万(-1.77%) "
               ...
               "2026-09-25 在发布基线上再现：SH601138、SH601899 的 06-29 与 07-01 各有两组值——"
               "SH601138 07-01 主力 -14.72亿(超大 -9.06亿、大 -5.66亿) 对 -14.74亿(超大 -9.31亿、"
               "大 -5.43亿)，06-29 大单 1022.63万 对 -487.52万，连符号都变；SH601899 07-01 主力 "
               "-1.87亿 对 -1.84亿，06-29 7.01亿 对 6.97亿。两组同样各自满足恒等式。"
               "东财网页 data.eastmoney.com/zjlx 的历史表调的就是同一个 push2his fflow/daykline，"
               "参数逐项相同（只差 JSONP 回调和时间戳）：SH601138 服务 18:13 与浏览器 18:34 各取的 "
               "120 行里，118 天的 12 个字段逐位相同，只有这两天不同，所以不是整份扰动副本。"
               "18:48 做了同一分钟交替对照（浏览器 cookie 身份对服务身份，3 轮）：第 1 轮 18:48:47 "
               "浏览器身份拿到新值、同一秒服务身份拿到旧值，后两轮两边都是旧值。所以不是按身份分"
               "（网页用的身份也会拿到新值），也不是整个接口一起翻（同一秒的两个请求可以不同），"
               "是逐次请求落到两个版本之一。SH601138 当天 8 次取数里新值 2 次，连同 08-28 基线和 "
               "09-22 两次共 11 次观测、旧值 9 次",
        gap="主力 0.1% ~ 9.1%，大单会变号",
        found="2026-09-12",
    ),
    PendingDisagreement(
    ...
    PendingDisagreement(
        what="财务表 · 年度净资产收益率（同花顺 对 新浪）",
        values="按报告期 join 比过 15 个标的、1029 个同期：最新报告期 15/15 一致，报告实际"
               "会渲染的那 90 个期里 2 个年度期不同——SZ002371 2024 年度 24.83% 对 20.63%、"
               "SH601318 2022 年度 10.10% 对 13.20%。新浪那个既不是同花顺的 ``净资产收益率``"
               " 也不是它的 ``净资产收益率-摊薄``（2024 年那期摊薄是 18.09%），是第三套口径，"
               "手上没有第三方能判对错。顺序固定 ths→sina，健康路径一份都不会变；只有同花顺"
               "没给的那一份报告来自新浪，且财务段自己会写明来路",
        gap="16.9% ~ 23.5%",
        found="2026-09-21",
    ),
)
