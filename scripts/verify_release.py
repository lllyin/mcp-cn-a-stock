...
PENDING_DISAGREEMENTS = (
    ...
    PendingDisagreement(
        what="板块资金流 · 行业板块的口径粒度（两个东财端点）",
        values="dataapi（降级源）m:90+t:2 返回 496 个，是申万三级树：BK0xxx 50 个 + "
               "BK1xxx 446 个，其中 82 个带 Ⅱ/Ⅲ 级后缀，父子同时在榜且重复计钱"
               "（证券Ⅱ 与 证券Ⅲ 都是 32.35亿；电子 -817.95亿 里含子板块 半导体 "
               "-602.46亿）；push2（主源）同样的 fs=m:90 t:2 只给一层扁平的东财行业板块",
        gap="行业板块数 496 对一层（本机 push2 不可达，主源那一侧的数没量到）",
        found="2026-09-05",
    ),
)
