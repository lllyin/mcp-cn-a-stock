...
文中的「云主机」指一台 2 核 4G、**机房出口 IP** 的 Ubuntu 服务器，「开发机」指
**家用宽带出口**的 macOS 笔记本。两者的差别不只是规格——机房 IP 和家宽 IP 在上游
风控那里待遇不同，所以浏览器身份、限流、验证码这几类结论**在两边可能相反**，不能
互相外推。凡是这一类的数字，本文都标出是在哪一类环境上量的；要用在自己的部署上，
请用 `scripts/probe_tuning.py` 在目标机器上重量一次。
带日期的数字是当时的实测记录，说明的是取值依据，不是承诺；换一台机器请用 `scripts/probe_tuning.py`
重测，它会给出这台机器自己的数。
...
重新发现一遍。2026-09-05 云主机日志里，少了这一步的代价是 26 秒内 18 次必败的 K 线
...
没有资金流向页面的标的（科创50 这类指数）在主源拒绝时原本没有第二条路：云主机 2026-09-06
的验证里，`SH000688` 的资金流因一次 `RemoteDisconnected` 整维缺失，占当轮 530 项里的 3 项。
现在 HTTP 这一层是一条 provider 链（`fund_flow_source.py`，`FUND_FLOW_PROVIDERS`，默认
`eastmoney,eastmoney_delay`）：`push2delay.eastmoney.com` 提供同一个 `fflow/daykline` 接口，
主源 `push2his` 对出口 IP 回空响应的同一时刻它仍应答，`1.000688` / `1.600519` / `0.399006`
的当日行与 `push2his` 逐字节相同；但 `lmt` 给多少都只回最近 1 行。所以它以 `complete=False`
的形态存在：编排层拿到它之后，有页面的标的仍去页面把 120 行历史补回来，没页面的标的至少有了
当日一行。主源通时行为逐字不变——同一个 AkShare 调用、同一张表。

没有页面的标的（科创50 这类指数）盘中另有一条路：push2delay 主机上的**分钟线**
`fflow/kline/get?klt=1`，每分钟一行，最后一行是当日累计的主力、小单、中单、大单、超大单净流入，
正是 zjlx 页面"今日"栏背后的接口。2026-09-07 10:16 实测科创50 46 行；日线在盘中对任何主机都没有当天
那一行，所以此前它盘中永远"暂无实时资金流向"。能力名 `realtime_fund_flow`，provider 仍是
`eastmoney_delay`，`REALTIME_FUND_FLOW_PROVIDERS` 接线。接口不给净占比，渲染时用净流入除以当日成交额
（上证指数核对 43.28 亿 / 3181.89 亿 = 1.36%，页面写 1.35%）。渲染顺序是 API 当天行 → 分钟线 → 暂无，
API 有当天行时输出逐字不变。有页面的标的不走这条路。
...
**浏览器身份用 main 分支的原样：无头、固定 Mac Chrome/120 的 UA、不带
`--disable-blink-features=AutomationControlled`、不注入、不改 locale。** 这条是在云主机上量出来的，
不是推理。2026-09-07 盘外两次交错 A/B，每种身份 36 次加载、同一批 6 只个股、同一判定（goto 后最多
等 22 秒，五个今日字段非占位算拿到）：

| 身份 | 今日拿到 | `fflow/kline/get` 断连 | `fflow/daykline/get` 断连 | checkuser 回 block |
| --- | ---: | ---: | ---: | ---: |
| main 原样 | 36/36 | 0/36 | 18/36 | 31 |
| 只加 AutomationControlled | 26/36 | 16/36 | 35/36 | 33 |
| 完整伪装（参数 + CDP 覆盖 UA/client hints + 注入脚本 + zh-CN） | 23/36 | 16/36 | 36/36 | 32 |

三个结论。一，东财 push2 的数据接口有一道身份门，对"明显的无头"放行、对"伪装过的"拒绝，只加那一个
启动参数就把 kline 断连从 0 打到 44%。二，滑块（`websitecaptcha/api/checkuser` 回 `block:true`）对三种
身份都弹，但不拦数据；拦数据的是接口那道门。三，页面脚本会在接口断连后自己重发：非 main 身份的
72 次里 12 次在首次断连后 1.9 到 3.7 秒内拿到了数据，所以 `_race_with_refusal` 在第一次断连之后再给
`REFUSAL_GRACE_SECONDS`（6 秒）而不是立刻判被拒。

2026-09-04 开发机实测曾得出相反结论（不加参数 2/16、加上 15/16），伪装那一套就是照开发机调出来的。
开发机与云主机面对的风控门不同：同一天云主机上 main 身份盘中命中 93%，开发机只有 54%。浏览器身份的
结论只能在云主机上测，开发机 A/B 不能外推。伪装实现保留在 `BROWSER_DISGUISE=1` 后面，默认关。

`FUND_FLOW_PAGE_MAX_LOADS` 默认 2，但阶梯改了：**被拒不 reload，直接换 tab**。同一个 tab 刷新带不走滑块，
新 tab 才带得走——云主机 09-07 上午首加载被拒后靠重试救回的 10 次里，新 tab 7 次、reload 3 次。reload 只留给
"没被拒只是没填完"的情形（冷启动、开盘前）。缺的那一块是接口拒的（今日到了、历史被拒，或反过来）也不再
加载，再来一次只是再被拒一次，拿到的那块直接交给调用方。云主机 .env 曾把预算调到 5 并把冷却压到 20 秒，
一个标的一次调用实时加历史两个调用方最多 10 次加载压在十几秒内，被拒 1.5 秒返回、间隔 300 毫秒，正是
09-07 上午滑块成批出现的放大器；预算回到 2。

实时那条路原先没有熔断器，滑块一出照样一个标的接一个标的地连发。现在浏览器层自己有一个
（`fund_flow_browser`），阈值、窗口、冷却与历史兜底那个共用同一组配置；只有"要今日却被拒"才计失败，
daykline 在 main 身份下也有一半被拒，历史缺失是常态，不能让它把实时停掉。brief 与 medium 不渲染历史表，
不再为它去打页面（`FetchRequirements.fund_flow_page`），只有 full 会。
...
2026-09-06 云主机实测，服务进程树在"热空闲"（最后一次调用 90 分钟内）是 621 MiB：
...
网关关闭时 K 线全走腾讯兜底，而它是最贵的一维。云主机 2026-09-06 两轮实测：verify 里
`_fetch_kline_sync` 68 次、service P50 8.27s，占全部上游耗时的 **79%**；60 次/分压测里
P50 12.5s，熔断已把东财跳掉之后仍然如此，所以慢的不是东财，是兜底路径本身。

拆开看单个标的（`SZ002875`，压测日志）：东财失败 2.0s，腾讯前复权 5.6s，腾讯不复权 5.9s。
腾讯那两段慢在请求数：`ak.stock_zh_a_hist_tx` 先打一次 `weekTrends` 查上市年份，再按
**日历年**循环，报告要两年历史就跨三个年份——一次复权取数 4 个请求，前复权加不复权 8 个，
云主机上每个约 1.9s。

腾讯接口本身按条数取（上限 640 根，从 `end` 往前数），两年只有约 480 根，一个请求就够。
所以 `platforms/tencent.py` 现在自己发请求：一页 640 根、超过才往前翻页；归一逐字照抄
AkShare（含它对 `sz000` 前缀不乘 100 的怪癖）；"认不认这个代码"也照抄它那道门——北交所
在 AkShare 里抛 KeyError 其实是 `weekTrends` 为空后硬取 `["day"]` 抛的，这道门保留下来，
北交所才继续由新浪服务（两家成交额精度不同：新浪到元、腾讯到万元）。门的结果按代码缓存，
稳态下每标的 8 个请求降到 2 个。

证明：`prove_equivalence.py` 在开发机跑 68 份文档，可判定 62 份 0 差异（开发机东财被拒，
K 线全走这条路，正是被改的路径）。第一版没保留那道门时 BJ920021 改判给了腾讯，成交量差 1 手、
成交额差 8 元——差异被比对抓出来，门才补上的。
...
它与数据源层解耦，`CACHE_ENABLED=0` 时完全不参与调用链。
