...
基于下游 57 天真实调用日志回放，标的级复用率 23.4%、代理积分降幅 23.3%。收益上限由需求
分布决定而非策略：58.5% 的请求发生在盘中。盘中重复查询的间隔呈双峰分布，60 秒内占 17.3%，
其余集中在 600 秒以上（中位 1777 秒），因此 `LIVE_TTL_SECONDS` 超过 60 秒几乎没有额外收益。
