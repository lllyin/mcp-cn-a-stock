...
没有资金流向页面的标的（科创50 这类指数）在主源拒绝时原本没有第二条路：部署机 2026-09-06
...
`--disable-blink-features=AutomationControlled`、不注入、不改 locale。** 这条是在部署机上量出来的，
...
2026-09-04 本机实测曾得出相反结论（不加参数 2/16、加上 15/16），伪装那一套就是照本机调出来的。
本机与部署机面对的风控门不同：同一天部署机上 main 身份盘中命中 93%，本机只有 54%。浏览器身份的
结论只能在部署机上测，本机 A/B 不能外推。伪装实现保留在 `BROWSER_DISGUISE=1` 后面，默认关。

`FUND_FLOW_PAGE_MAX_LOADS` 默认 2，但阶梯改了：**被拒不 reload，直接换 tab**。同一个 tab 刷新带不走滑块，
新 tab 才带得走——部署机 09-07 上午首加载被拒后靠重试救回的 10 次里，新 tab 7 次、reload 3 次。reload 只留给
"没被拒只是没填完"的情形（冷启动、开盘前）。缺的那一块是接口拒的（今日到了、历史被拒，或反过来）也不再
加载，再来一次只是再被拒一次，拿到的那块直接交给调用方。部署机 .env 曾把预算调到 5 并把冷却压到 20 秒，
...
网关关闭时 K 线全走腾讯兜底，而它是最贵的一维。部署机 2026-09-06 两轮实测：verify 里
...
证明：`prove_equivalence.py` 在本机跑 68 份文档，可判定 62 份 0 差异（本机东财被拒，
