...
# 就打一条 WARNING。置 0 关闭。
#
# 开着有明确代价：正常路径上 resolve() 问到第一个就停，开了之后每个标的
...
#
# 默认值 8% 的依据（两次实测定值，不拍脑袋）：已知的最大跨源分歧是创业板指成交量
# 腾讯 vs 同花顺——2026-09-07 量得 3.885%、2026-09-12 盘中量得 5.45%，这个分歧随
# 市场浮动，阈值必须盖过它的上沿，否则每次调用都在为一个已裁决的差异刷 WARNING；
# 而这个校验真正要抓的——成交量单位错档（100 倍）、字段取错位、源返回陈旧数据——
# 偏差都在几十个百分点以上，8% 离它们很远。其余标的（个股/ETF/其他指数）当日实测
# 跨源偏差全部 <0.01%。代价不变：每个标的多一次网络源请求。
INTRADAY_QUOTE_CROSS_CHECK_PCT = max(
    0.0,
    float(env("INTRADAY_QUOTE_CROSS_CHECK_PCT", "8")),
)
