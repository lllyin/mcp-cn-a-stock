...
#
# 默认值 5% 的依据（2026-09-11 实测定值，不拍脑袋）：已知的最大跨源分歧是创业板指
# 成交量 3.885%，阈值必须盖过它，否则每次调用都在为一个已裁决的差异刷 WARNING；
# 而这个校验真正要抓的——成交量单位错档（100 倍）、字段取错位、源返回陈旧数据——
# 偏差都在几十个百分点以上，5% 离它们很远。当日实测 9 个标的（个股/ETF/指数）
# 跨源偏差全部 <0.01%，5% 的余量充足。代价不变：每个标的多一次网络源请求。
INTRADAY_QUOTE_CROSS_CHECK_PCT = max(
    0.0,
    float(env("INTRADAY_QUOTE_CROSS_CHECK_PCT", "5")),
)
