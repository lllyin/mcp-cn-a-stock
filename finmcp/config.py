...
#
# 2026-09-05 在 2 核 4G 云主机上实测把它推到 3：那一轮 5 次丢失**全部**是"名额已满"，一次
...
#     4   11/11   多救回 4 个
#
# 取 3 不取 4：4 要把浏览器页面上限也推到 4，多两个并发渲染进程，而第 4 个页面能
# 多救回多少没量过——收益说不清就不动（§五）。
# 内存那笔要说清落在哪：目标环境实测的 PSS 峰值由**浏览器实例本身**决定，与同时开
# 第 3 还是第 4 个页面不是一个量级的变量（个位数加载与上百次加载的峰值同量级）。
# 所以这一项该按"救回多少等待"来定，不是按"会不会撑爆内存"来定；内存超没超 §四 是
# 实例在不在跑的问题，见 verify_release 报告里单列的"其中浏览器 / 服务本体"两行。
# 必须和 BROWSER_MAX_PAGES 一起提——只提这一个，浏览器信号量会立刻变成新的瓶颈，
# 收益为零。
#
...
# 可以退回"不等，直接跳过"。
#
# 2026-09-05 实测把它从 3s 提到 8s。判据是 W 和 hold 的关系，不是名额——名额只决定
# "前几个能立刻拿到"。**hold 随机器变，所以这一项必须在目标环境上量**：2 核 4G 云主机
# 上 hold 是 p50 3.38s / p90 7.49s，而页面加载快的开发机只有 2.26s——在快的机器上
# 这个问题量不出来。按慢的那一侧：
#
...
# 实时页面加载（require_today）的熔断阈值：窗口内多少次被拒后暂停实时那条路。
#
# 只管实时路径。历史兜底（require_history）不用这组值也不熔断——它是付费网关
# 之前的最后一级免费途径，必须每次都真试。
#
...
# 免费的同步链立刻对齐，空探测自然作废。取值：跑 probe_tuning 的 fund-flow-landing
# 项，它按"同一（标的, 目标日, 腿）先探到行不存在、后拿到行"的配对给出**最大值**
# 和证据——按最大值是为了不砍掉落地最慢的那一截，那正是要保住的数据。没有样本时
# 保持默认或往小调：取小了只是多付一次探测，取大了会把已经落地的行挡在有效期外，
# 那是数据变少。
...
# **这个结论会随出口 IP 反过来**：另一个出口上量到的是"不加参数 2/16、加上 15/16"，
# 和上表正好相反。不同出口面对的风控门不同，所以浏览器身份这一项只能在**将要部署的
# 那个出口**上量，别照搬任何一份现成的 .env（scripts/probe_tuning.py 就是干这个的）。
...
#   real  照实报，用于在目标部署环境上做对照
# 注意代价：声明 macOS 之后 WebGL renderer 和字体列表仍是 Linux 的样子，若对端
# 交叉核对到那一层，声明 macOS 反而更可疑。所以要在目标部署环境上用 blocked_captcha
...
# 收益没有实测数据支撑——量它的时候出口 IP 正处于持续封锁态，命中率变化量不出来；写下这一点
...
#
# 原值 2 的由来是小规格部署：2 核 4G 上建议并发不超过 2,那是 CPU 侧的约束——每个
...
# 盘外（收盘后、非交易日）的空闲回收超时,秒。置 0 = 盘外不回收（盘中照旧）。
# 注意上面那个是总开关:它置 0 就整层关掉,不看时段。
#
# 为什么要分成两个值:上面那个 90 分钟的唯一依据是"盖住午休",而午休只在盘中存在。
# 收盘后到次日开盘有 18 小时、周末更长,同样空转 90 分钟纯属白占——2026-09-06 在
# 2 核 4G 云主机上实测,热空闲时服务进程树 621 MiB,其中浏览器那四个进程（driver 111 +
# gpu 102 + 主进程 93 + network 72）合计 378 MiB,占 61%;拆掉之后只剩 243 MiB。
#
# 收益:盘外约 18 小时/天加整个周末,常驻从 621 MiB 降到 243 MiB。
# 代价:盘外空闲后第一个请求多付 2.19s（冷 2.88s vs 热 0.69s）。
# 两者都有量级,且不在同一个数量级上,所以分档。
#
# 取值时机是**排定时器的那一刻**（_arm_idle_timer）,不是到期时。所以 15:59 排下的
# 定时器会按盘中口径留到 17:29——每天最多多留一个窗口。换成到期时再判就要引入
# 周期性唤醒,而这个定时器刻意做成"没有借用者时才存在",空闲期一次也不唤醒。
BROWSER_IDLE_TIMEOUT_CLOSED_SECONDS = max(
    0.0,
...
# 盘中行情跨源校验的相对偏差告警阈值（百分比），默认 0 关闭。
# 排查数据口径时按当前环境设置阈值；开启会串行请求所有剩余来源，增加延迟和请求数。
INTRADAY_QUOTE_CROSS_CHECK_PCT = max(
    0.0,
    float(env("INTRADAY_QUOTE_CROSS_CHECK_PCT", "0")),
)

# 跨源校验这一整轮的总预算（秒），0 表示不限。
#
# 为什么要有：校验挡在报价返回之前，要问完剩下的**所有**源。没有总预算的话，一个
# 挂住的源就给每个标的加满一份单次超时，而这只是个观察点——正是 AGENTS §三 点名的
# 「N × 单次超时」结构。预算会穿给各 provider，让它们把自己的超时压到剩余量以内，
# 所以这里是真正的上限，不是"调小单次超时换个系数"。
#
# 默认 2 秒的依据（实测定值）：三个网络源健康时单次都在 0.14s 以内，问完一轮约
# 0.4s，2 秒是它的五倍余量，够盖住网络抖动和冷 DNS；而校验只在显式开启时才跑。
# 怎么为自己的环境取值：按**最大值**定不按分位数——分位数按定义会砍掉尾部，而
# 尾部正是这个预算要兜住的东西。量一轮各源的单次最大耗时，乘上源的个数再留余量。
INTRADAY_QUOTE_CROSS_CHECK_BUDGET_SECONDS = max(
    0.0,
    float(env("INTRADAY_QUOTE_CROSS_CHECK_BUDGET_SECONDS", "2")),
)

# 交易日历的进程内缓存时长。交易日历是提前一年公布的，一天刷一次绰绰有余；
# 取一次实测 0.18s / 8797 行 / 69 KiB，缓存住之后对每次判断是零成本。
CACHE_CALENDAR_TTL_SECONDS = max(
    0.0,
    float(env("CACHE_CALENDAR_TTL_SECONDS", "86400")),
)

# 板块分级表的进程内缓存时长。行业分类一年动一两次，一天刷一次绰绰有余；
# 取一次实测 31+131 行、约 2.4s，缓存住之后对每次查询是零成本。
CACHE_TAXONOMY_TTL_SECONDS = max(
    0.0,
    float(env("CACHE_TAXONOMY_TTL_SECONDS", "86400")),
)
...
# 置 0 关闭这道闸门。
#
# 防的是**序列断裂**：缺列会让契约当场判失败，而断裂的序列列是齐的、数值也都在
# 合理区间，源「成功」返回，没有任何东西会拦它。实际发生过：SH000688 的涨跌幅报成
# +20.17%（真实 +2.41%），成因是同花顺某个年份文件取失败被静默
# 跳过，今天前面那一根从 1577.36 变成了 1344.07；同一份报告 240 日均价 1540 → 1148、
# 240 日最高 2255 → 1626（等于当天最高，历史高点整段没了）。
#
# **单位是交易日不是自然日**，这一点是关键。第一版数自然日、阈值 15，那只是「别把
# 长假当缺口」的代理指标：得坐在最长连休之上又远低于丢掉一整年。实测余量很薄——
# 2015 年以来春节最长 11 个自然日，对 15 只剩 4 天。改成问交易日历之后长假在结构上
# 就是 0（33 条真实序列实测缺 0 个交易日），阈值只剩一个真问题要回答：容忍几天停牌。
#
# 10：2018 年之后重大资产重组停牌基本以 10 个交易日为上限，合规范围内的停牌不触发
# 额外那一轮回退；而漏一个年份文件是 242 个交易日、漏一个季度 60，都远在阈值之上。
# 日历取不到时退回按星期数，最长的长假被数成 6，仍在 10 之内。
#
# 这是**偏好不是硬条件**：真实的长期停牌会让每个源都给出同样带缺口的序列，那时候
# kline_source.resolve() 会宽松再问一轮、放行带缺口的序列并留下 kline_gap_tolerated。
...
# `__file__/../` 指到 site-packages，那里没有 confs——**装成包运行时** SH_INDICES
# 因此为空，SH000001 被"纠正"成 SZ000001，上证指数报成了平安银行。
# 开发时跑 `python main.py`，包就在仓库里，三种找法都命中，照不出这个洞：
# 这个 bug 只在 `pip install` 之后的部署形态下出现。
#
