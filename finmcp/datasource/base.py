...
class FetchRequirements:
    ...
    #: 资金流历史取不到时，允不允许去浏览器页面补。由配置链推导（mcp_app：
    #: FUND_FLOW_PROVIDERS 里配了 fund_flow_page 就为真），不再按工具类型砍——
    #: 页面是付费网关前的最后一级免费途径，brief/medium 的当日资金流同样先过它。
    ...
    #: 这次渲染要不要历史资金流向表。只有 full 渲染它——所以 full 的资金流需求里
    #: 行数恰恰是需求（哪怕钉了日期也一样）；brief/medium 不渲染，钉日期时只需要
    #: 命中那一天，行数不是需求。
    fund_flow_history_table: bool = True
